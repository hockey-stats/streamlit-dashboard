...
def main():
    ...
    print(f"--- Starting Public Data Extraction for {date_str} ---")
    ...
    try:
        ...
        if not team_stats.is_empty():
            team_stats.write_csv("data/team_stats.csv")
            print(f"Saved data/team_stats.csv ({len(team_stats)} teams)")
    ...
    try:
        ...
        if not pitcher_stats.is_empty():
            pitcher_stats.write_csv("data/all_pitcher_stats.csv")
            print(f"Saved data/all_pitcher_stats.csv ({len(pitcher_stats)} pitchers)")
    except Exception as e:
    ...
    try:
        ...
        if not probables.is_empty():
            probables.write_csv("data/todays_probables.csv")
            print(f"Saved data/todays_probables.csv ({len(probables)} games)")
