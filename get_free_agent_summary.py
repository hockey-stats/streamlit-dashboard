...
def get_free_agents(position: str, league: yfa.League) -> pl.DataFrame:
    ...
    player_ids = [
        int(player["player_id"]) for player in players if player["status"] == ""
    ]
...
def get_players_from_own_team(
    position: str, league: yfa.League, session: OAuth2
) -> pl.DataFrame:
    """
...
def collect_pitcher_stats(
    player_ids: list, league: yfa.League, position: str
) -> pl.DataFrame:
    """
    ...
    for player_stats, term in zip(
        [player_stats_week, player_stats_month, player_stats_season],
        ["week", "month", "season"],
    ):
        for details, stats in zip(player_details, player_stats):
            ...
            p_dict["team"].append(
                standardize_team_names(details["editorial_team_abbr"])
            )
            p_dict["positions"].append(
                ",".join(
                    [
                        pos["position"]
...
def filter_free_agents(
    fa_df: pl.DataFrame, t_df: pl.DataFrame, position: str
) -> pl.DataFrame:
    """
...
def filter_taken(
    df: pl.DataFrame, free_agents: list[int], my_team: list[int]
) -> pl.DataFrame:
    """
...
def compute_z_scores(batter_df: pl.DataFrame, player_type: str) -> pl.DataFrame:
    ...
    for term in ["season", "month", "week"]:
        ...
        x = x.with_columns(
            pl.struct("z_total").rank("ordinal", descending=True).alias("Rank")
        )
...
def main() -> None:
    ...
    my_team = list(
        get_players_from_own_team(position="Util", league=league, session=session)["id"]
    ) + list(
        get_players_from_own_team(position="P", league=league, session=session)["id"]
    )
